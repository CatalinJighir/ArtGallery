...
describe("Details", () => {
  it("renders correctly", () => {
    const { getByText } = render(<Details id={"1"} details={mockDetails} />);
  ...
  it("should not render when fonts are not loaded", () => {
    ...
    const { queryByText } = render(<Details id={"1"} details={mockDetails} />);
  ...
  it("should not display the buy button if current owner is the user", () => {
    ...
    const { queryByText } = render(<Details id="1" details={customDetails} />);
