...
const ArtForm = ({ setMode }: { setMode: Function }) => {
  ...
  const checkInputs = () => {
    ...
    if (!licenseType || licenseType === "") {
      Toast.show({
        type: "warning",
        text1: "Please select a license type",
      });
      return false;
    }
  ...
  const publishArt = async () => {
    ...
    try {
      ...
      Toast.show({
        type: "success",
        text1: "Art published successfully",
      });
      setMode("view");
    } catch (err) {
      setRequestPending(false);
      Toast.show({
        type: "error",
        text1: "Failed to publish the art",
      });
    }
  };
  ...
  return (
    <Wrapper>
      ...
      <ImageInputWrapper>
        {imageUri.uri === "" ? (
          <PlusIconWrapper onPress={handleUpload}>
            <PlusIcon name="plus" size={90} color="#fff" />
          </PlusIconWrapper>
        ) : (
      ...
      <PublishButton onPress={publishArt} disabled={requestPending}>
        <LinearGradient
